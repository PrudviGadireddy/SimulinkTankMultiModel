load('trained_models/single_class_objdet_gTruth.mat')
...
%try using resnet18 trained on other tanks

%network = load("resnet18_500epochs_tanks.mat").netTransfer;
%try loading pretrained squeeznet from other tank source
network = load("trained_models/squeezenet_pretrainedRU-UKR_80epochs_noLabelcnsldt.mat").netTransfer;
...
%featureLayer = "res4b_relu";%"activation_40_relu";
...
%%
%% Load and Deploy Pretrained Model
% detector = load("trained_models/yolov3_squeezepretrained_220epochs_2022b(1).mat").detector
...
if size(scores) ~= 0
    ...
    resultImage = insertObjectAnnotation(img_in, 'rectangle', bboxes(max_idx,:), scores(max_idx));
    imshow(resultImage);
else
    imshow(img_in)
end
...
% PC Model
[bboxes_pc,scores_pc,labels_pc] = detect(detector,img_in,Threshold=0.4);
figure
if size(scores_pc) ~= 0
    result_pc = insertObjectAnnotation(img_in,"rectangle",bboxes_pc,scores_pc);
    imshow(result_pc)
else
    imshow(img_in)
end
...
threshold = 0.05;
results = detect(detector,preprocessedTestData, MiniBatchSize=4, Threshold=threshold);
[am, fppi, missRate] = evaluateDetectionMissRate(results,preprocessedTestData, threshold);
[ap, precision, recall] = evaluateDetectionPrecision(results, preprocessedTestData,threshold);
%%
